...
let rec evaluate_conv ~verbose reduce t =
  if verbose then print_string (format_term_conv t) else ();
  match reduce t with
  | None ->
    if verbose then print_string " =/=>\n\n" else ();
    t
  | Some t' ->
    if verbose then print_string " ==>\n\n" else ();
    evaluate_conv ~verbose reduce t'

let test_conv ~verbose ~sem ~reduce s =
  printf "\nEvaluating:\n%s\nin %s semantics:\n\n" s sem;
  let result = (evaluate_conv ~verbose reduce (parse_conv s)) in
  printf "Result is: %s\n\n" (format_term_conv result)

let test_lazy_conv = test_conv ~sem:"lazy" ~reduce:reduce_lazy
let test_strict_conv = test_conv ~sem:"strict" ~reduce:reduce_strict
let test_normal_conv = test_conv ~sem:"normal-order" ~reduce:reduce_normal
let test_all_conv ~verbose s =
  test_lazy_conv ~verbose s;
  test_strict_conv ~verbose s;
  test_normal_conv ~verbose s

 let test_times = env ^ "
((equal ((times ((times c2) c2)) ((((times c2))) (((c2)))))) ((plus c8) c8))
"
 let test_times_conv = env ^ "
equal ((((times))) (times c2 c2) (times c2 c2)) (plus c8 c8)
"
let () =
  ...
  test_all ~verbose:false test_times;
  test_all_conv ~verbose:false test_times_conv;
  test_all ~verbose:true "(let x = (\\f.f) in (x))";
  test_all ~verbose:true "(let x = x in (x))";
  test_all_conv ~verbose:true "(let x = x in ((let x = \\x.x in (x x x x)) x))";
  test_all_conv ~verbose:true "let x = \\t.let y =\\f.f in y in x";
  test_all_conv ~verbose:true test_and_1;
  test_all_conv ~verbose:true test_and_2;
  test_lazy_conv ~verbose:false (format_term_conv (parse_conv test_fact_l));
  test_lazy ~verbose:false (format_term (parse_conv test_fact_l))